 ...
 */
public class Activity6463 extends Activity implements View.OnClickListener {
    ...
    private static final int CLOUD_CONNECTED = 2;
    private static final int CLOUD_UNCONNECT = 0;
    private static final int CLOUD_LOST = 1;
    ...
    private boolean tcpConnected = true;
...
//        dataText = (TextView) findViewById(R.id.textdata);
        ...
        simpleAdapter = new SimpleAdapter(Activity6463.this, maps, R.layout.list_item, new String[]{MAC, DEPTH, NETSTATE, MODE, ATTR},
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, final int i, long l) {
                final Dialog dialog = new Dialog(Activity6463.this);
                View dialogView = LayoutInflater.from(Activity6463.this).inflate(R.layout.dialog, null);
...
//            dataText.setText(dataText.getText().toString() + "rec id=" + receiveData.id + "  " + str);
                totalLen = receiveData.totalLen;
                if (totalLen == -1) {
                    showToast("sdk返回数据总长度是-1");
                    return;
                }
                if (serialSvrBinder == null) {
                    showToast("serialSvrBinder=null");
                ...
                switch (receiveData.id) {
                    ...
                    case 27://设置密钥返回
                        if (!is64) {
                            ...
                            Log.d(SerialSvr.tag, "set psk success 63 subscribe:");
                            subscribe(mac63);
                        }
                        showToast("设置密钥成功");
                        break;
    ...
    private void handleSearchAtt(MqttNativeInterface.ReceiveData receiveData) {
        ...
        mac64 = receiveData.id47_srcMac;
        byte taskId = receiveData.id47_taskid;
        MqttNativeInterface.SearchResp searchResp = new MqttNativeInterface().new SearchResp();
        searchResp.src = mac64;
        searchResp.taskid = taskId;
...
//                    keepAlive();
            ...
            case suback:
                String mmac = null;
                if (mac63Str == null) {
                    mmac = getMacStr(mac64);
                    Log.d(SerialSvr.tag, "receive 64 suback data and publish " + mmac);
                } else {
                    mmac = mac63Str;
                    Log.d(SerialSvr.tag, "receive 63 suback data and publish " + mmac);
    ...
    private void mqttConnect() {
        byte[] connect = MqttManeger.connect(clientId, userName, psd);
...
//                showToast("tcp连接云unconnect");
    ...
    private void sendTcpLink() {
        MqttNativeInterface.TcpData tcpData = new MqttNativeInterface().new TcpData();
        tcpData.loc_ip = new byte[]{0, 0, 0, 0};
        tcpData.loc_port = 0;
        tcpData.rmt_ip = rmtIp;
        tcpData.rmt_port = rmtPort;
    ...
    private void handleMac(MqttNativeInterface.ReceiveData receiveData) {
        ...
        if (is64) {
            ...
            mac63 = receiveData.id23_mac;
//            showToast(getStr(mac63));
        }
    }

    private void handleVer(MqttNativeInterface.ReceiveData receiveData) {
        short config = receiveData.id22_conf;
        byte c = (byte) config;
        byte cc = (byte) ((c >> 4) & 0x1);
        if (cc == 1) {
            is64 = true;
            ly64.setVisibility(View.VISIBLE);
        }
    ...
    private void publish(String mac, String message) {
        ...
        if (is64) {
            ...
            MqttNativeInterface.PlcData plcData = new MqttNativeInterface().new PlcData();
            plcData.dst = mac64;
            plcData.data = pubBytes;
            plcData.len = plcData.data.length;
            serialSvrBinder.write(mqttNativeInterface.sdk_tx_plc_req(plcData));
        }
    }

    private void subscribe(byte[] mac) {
        byte[] subBytes = MqttManeger.subscribe(1, TOPIC_TITLE + getMacStr(mac) + TOPIC_TAIL, (short) (subMessageId + 1));
        MqttNativeInterface.PlcData plcData = new MqttNativeInterface().new PlcData();
        plcData.dst = mac64;
        plcData.data = subBytes;
        plcData.len = plcData.data.length;
        serialSvrBinder.write(mqttNativeInterface.sdk_tx_plc_req(plcData));
    }
    ...
    private ServiceConnection serialConn = new ServiceConnection() {
        ...
        public void onServiceDisconnected(ComponentName name) {
            Toast.makeText(Activity6463.this, "--Service Disconnected--", Toast.LENGTH_SHORT).show();
        }
    };
...
//                dataText.setText("");
    ...
    private void clickLight() {
        if (is64 && mac64 == null || !is64 && mac63 == null) {
            showToast("mac为空");
            return;
        }
        LightGson lightGson = new LightGson();
        ...
        publish(is64 ? getMacStr(mac64) : getMacStr(mac63), message);
    }
...
//        dataText.setText("");
        byte[] ver = mqttNativeInterface.sdk_get_ver_req();
//        showToast(getStr(ver));
        if (serialSvrBinder == null) {
            showToast("serialSvrBinder=null");
            return;
        }
        serialSvrBinder.write(ver);
    }

    private void showToast(String str) {
        Toast.makeText(Activity6463.this, str, Toast.LENGTH_SHORT).show();
    }
